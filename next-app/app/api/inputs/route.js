...
export async function GET() {
  try {
    const res = await fetch(`${BACKEND_URL}/api/inputs`, {
      cache: 'no-store',
      next: { revalidate: 0 },
    });
    if (res.ok) {
    ...
    console.warn('[Next API Proxy] Error fetching /api/inputs:', err);
  }
...
export async function POST(request) {
  try {
    ...
    console.error('[Next API Proxy] Error saving /api/inputs:', err);
  }
